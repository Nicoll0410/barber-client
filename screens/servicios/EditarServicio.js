...
const EditarServicio = ({ visible, onClose, servicio, onUpdate }) => {
  ...
  useEffect(() => {
    if (servicio) {
      ...
      setPrecio(servicio.precio.replace('$ ', ''));
      setInsumos(servicio.insumos || []);
    }
  }, [servicio]);
    ...
    <Modal visible={visible} animationType="fade" transparent={true}>
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <ScrollView contentContainerStyle={styles.scrollContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Editar insumos del servicio</Text>
              <TouchableOpacity onPress={() => setPaso(1)}>
                <Ionicons name="arrow-back" size={20} color="#777" />
              </TouchableOpacity>
            </View>

            <Text style={styles.subtitle}>Edita los insumos requeridos</Text>
...
const styles = StyleSheet.create({
  ...
  modalView: {
    width: '40%', // Mismo ancho que CrearServicio.js
    maxHeight: '80%',
